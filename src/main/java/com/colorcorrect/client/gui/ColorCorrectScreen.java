...
public class ColorCorrectScreen extends Screen {
	...
	public ColorCorrectScreen(Screen parent, String initialCategory) {
	...
	protected void init() {
		...
		for (String cat : CATEGORIES) {
			addDrawableChild(ButtonWidget.builder(Text.translatable("colorcorrect.category." + cat), b -> {
				// IMPORTANT: never rebuild this screen's children from inside a
				// widget callback — the screen's input handlers are iterating the
				// children list while the press event is being dispatched, and
				// clearChildren()/init() would throw a ConcurrentModification-
				// Exception. Open a fresh screen instance instead (the pattern
				// vanilla uses for its own tabbed option screens).
				this.client.setScreen(new ColorCorrectScreen(this.parent, cat));
